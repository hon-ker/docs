...
![image-20240115162854134](./assets/image-20240115162854134.png)

```powershell
net user clay // 查看用户的详细信息
```

![image-20240115163113755](./assets/image-20240115163113755.png)

```powershell
net user clay passwd 		// 修改用户密码
net user clay passwd /add 	// 添加新用户
net user clay$ passwd /add 	// 添加新用户
```
...
net localgroup administrators clay /add 	// 把clay用户添加到administrators组
net localgroup 组名 /add					   // 新建组名
```

![image-20240115163749192](./assets/image-20240115163749192.png)

## 0x02 Potato提权

提权的目的，应该是为了更好的攻击，拥有更多的攻击面，更好的攻击



Potato提权就是**通过各种方式在本地（NTLM-relay）中获得system令牌，再通过模拟令牌执行命令**
...
```bash
1. 获取高权限令牌 --token # 令牌
2. SeImpersonatePrivilege 或SeAssignPrimaryTokenPrivilege 特权	 # 模拟
```
...
- 本地管理员账户或者本地服务账户（不包括本地管理员组普通用户）
- 由SCM（服务控制管理器）启动的服务
- 由组件对象模型（COM）基础结构启动的并配置为在a特定账户下运行的COM服务器
...
![image-20240115165903112](./assets/image-20240115165903112.png)



为什么需呀那些权限（SeImpersonatePrivilege 或SeAssignPrimaryTokenPrivilege ）？

```bash
# 当有SeImpersonatePrivilege
当有SeImpersonatePrivilege 权限时，可以调用CreateProcessWithTokenW，以某个token 的权限去启动进程
# SeAssignPrimaryTokenPrivilege
当有SeAssignPrimaryTokenPrivilege 权限时，可以调用CreateProcessAsUserW， 以h指定用户权限启动新进程
```

windows的Token：

- Delegation token(授权令牌) 用于交互会话登陆
- Impersonate Token (模拟令牌) 用于非交互登陆





windows上的每一个服务，每一个账号都映射一个身份，一个权限，下面是windows常见服务使用的账号

- NT AUTHORITY\SYSTEM2
- NT AUTHORITY\NETWORK SERVICE
- NT AUTHORITY\LOCAL SERVICE



![image-20240115171336331](./assets/image-20240115171336331.png)
...
## 0x03 系统错误配置提权 AlwaysInstallElev
